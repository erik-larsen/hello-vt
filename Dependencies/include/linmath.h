//
// linmath.h - 3D math library
//
// Based on https://github.com/datenwolf/linmath.h
//
// Provides the most used types required for programming computer graphics:
...
//
// Additional funcs Jan 2025 LibVT 
//
LINMATH_H_FUNC float clamp(float value, float min, float max)
{
    return value < min ? min : (value > max ? max : value);
}

LINMATH_H_FUNC void mat4x4_print(mat4x4 M) {
    for (int i = 0; i < 4; i++)
        printf("[%f, %f, %f, %f]\n", M[i][0], M[i][1], M[i][2], M[i][3]);
}

LINMATH_H_FUNC void mat4x4_scale_iso(mat4x4 M, mat4x4 const source, float s) {
    mat4x4_scale(M, source, s);
    M[3][3] = 1.0f;
}
#endif
